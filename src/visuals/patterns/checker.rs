...
impl Pattern for CheckerPattern {
    fn color_at(&self, point: Point) -> Color {
        if ((point.x.round() + point.y.round() + point.z.round()) as isize) % 2 == 0 {
...
mod test {
    ...
    #[test]
    fn color_at_repeats_in_x() {
        let color_a = Color::new(0.0, 0.0, 0.0);
        let color_b = Color::new(1.0, 1.0, 1.0);
        let pattern = CheckerPattern::new(color_a, color_b, None);

        assert_eq!(pattern.color_at(Point::new_point(0.0, 0.0, 0.0)), color_a);
        assert_eq!(pattern.color_at(Point::new_point(0.49, 0.0, 0.0)), color_a);
        assert_eq!(pattern.color_at(Point::new_point(0.51, 0.0, 0.0)), color_b);
        assert_eq!(pattern.color_at(Point::new_point(1.51, 0.0, 0.0)), color_a);
    }

    #[test]
    fn color_at_repeats_in_y() {
        let color_a = Color::new(0.0, 0.0, 0.0);
        let color_b = Color::new(1.0, 1.0, 1.0);
        let pattern = CheckerPattern::new(color_a, color_b, None);
