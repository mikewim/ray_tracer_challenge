...
pub fn generate_3d_sphere(path: &str) -> Result<(), Error> {
    ...
    // sphere.transform = Matrix::scaling(1.0, 0.5, 1.0);
    // sphere.transform = Matrix::translation(1.0, 0.0, 0.0);

    let light_position = Coordinates::new_point(-10.0, 10.0, -10.0);
        ...
        for x in 0..canvas_pixels {
            ...
            if let Some(intersections) = sphere.intersect(ray) {
                if let Some(hit) = hit(intersections) {
                    ...
                    let surface_normal = hit.object.normal_at(point);
