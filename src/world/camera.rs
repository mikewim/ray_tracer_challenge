use super::World;
use crate::base_types::{Coordinates, Ray};
...
impl Camera {
    ...
    pub fn ray_for_pixel(self, x: usize, y: usize) -> Ray {
        ...
        let pixel = camera_transform_inv * Coordinates::new_point(world_x, world_y, -1.0);
        let origin = camera_transform_inv * Coordinates::new_point(0.0, 0.0, 0.0);
...
#[cfg(test)]
mod test {
    use super::{Camera, Coordinates, Matrix, World};
    ...
    fn pixel_size() {
        ...
        assert!(float_equal(horiz_camera.pixel_size, 0.0099999));

        let vert_camera = Camera::new(125, 200, PI / 2.0);

        assert!(float_equal(vert_camera.pixel_size, 0.0099999));
    }
    ...
    fn ray_for_pixel_center() {
        ...
        assert_eq!(ray.origin, Coordinates::new_point(0.0, 0.0, 0.0));
        assert!(ray.direction.equal(Coordinates::new_vector(0.0, 0.0, -1.0)));
    }
    ...
    fn ray_for_pixel_corner() {
        ...
        assert!(ray
            .direction
            .equal(Coordinates::new_vector(0.66519, 0.33259, -0.66851)));
    }
    ...
    fn ray_for_pixel_with_transform() {
        ...
        assert_eq!(ray.origin, Coordinates::new_point(0.0, 2.0, -5.0));
        assert!(ray.direction.equal(Coordinates::new_vector(
            (2.0 as f64).sqrt() / 2.0,
            0.0,
            -(2.0 as f64).sqrt() / 2.0
        )));
    ...
    fn render_default() {
        ...
        let from = Coordinates::new_point(0.0, 0.0, -5.0);
        let to = Coordinates::new_point(0.0, 0.0, 0.0);
        let up = Coordinates::new_point(0.0, 1.0, 0.0);
