...
impl Matrix {
    ...
    pub fn inverse(self) -> Option<Self> {
        ...
        for i in 0..4 {
            for j in 0..4 {
                rows[i][j] = self.cofactor(i, j);
            }
        }

        let inv_matrix = Self::new_with_rows(rows).transpose();
        Some(inv_matrix / determinant)
    }
...
impl ops::Mul for Matrix {
    ...
    fn mul(self, _rhs: Self) -> Self {
        ...
        for i in 0..4 {
            for j in 0..4 {
                new_rows[i][j] = self.rows[i][0] * _rhs.rows[0][j]
...
impl ops::Mul<Coordinates> for Matrix {
    type Output = Coordinates;

    fn mul(self, _rhs: Coordinates) -> Coordinates {
        let mut new_coords = Coordinates(0.0, 0.0, 0.0, _rhs.3);

        new_coords.0 = self.rows[0][0] * _rhs.0
            + self.rows[0][1] * _rhs.1
            + self.rows[0][2] * _rhs.2
            + self.rows[0][3] * _rhs.3;

        new_coords.1 = self.rows[1][0] * _rhs.0
            + self.rows[1][1] * _rhs.1
            + self.rows[1][2] * _rhs.2
            + self.rows[1][3] * _rhs.3;

        new_coords.2 = self.rows[2][0] * _rhs.0
            + self.rows[2][1] * _rhs.1
            + self.rows[2][2] * _rhs.2
            + self.rows[2][3] * _rhs.3;
...
impl ops::Div<f64> for Matrix {
    ...
    fn div(self, _rhs: f64) -> Self {
        ...
        for i in 0..4 {
            for j in 0..4 {
                new_rows[i][j] = self.rows[i][j] / _rhs;
            }
        }
...
mod test {
    ...
    fn mul() {
        ...
        assert_eq!(Matrix::new_with_rows(prod_rows), matrix1 * matrix2);
    }
    ...
    fn mul_vec() {
        ...
        let vec = Coordinates::new_vector(2.0, -1.0, 4.0);

        assert_eq!(
            Matrix::new_with_rows(rows) * vec,
            Coordinates::new_vector(12.0, 32.0, 38.0)
        );
    }
    ...
    fn inverse() {
        ...
        assert!(matrix1.equal(&(product_matrix * matrix2_inv.unwrap())));
    }
}
